...
public interface PersonsitoryInterface {
    ...
    List  inSalaryRange(int start, int end); // Liệt kê danh sách những người có mức lương trong khoảng được chỉ định start -> end

    List<Person> startsWith(String prefix);

    List  sortByBirthYearDescending(); // Sắp xếp danh sách theo năm sinh giảm dần
